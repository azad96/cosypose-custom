...
def main():
    ...
    # folder_name = 'object_mix3'
    # folder_pth = '/mnt/trains/users/azad/BM/bracket_mix/new_{}'.format(folder_name)
    # save_dir = '/mnt/trains/users/azad/BM/new_results/{}'.format(folder_name)
    # save_file = 'coarse'
    # version = 'v2' 
    
    folder_name = 'rgb_object'
    folder_pth = '/mnt/trains/users/azad/BM/inputs/bracket/{}'.format(folder_name)
    save_dir = '/mnt/trains/users/azad/BM/results/bracket/{}'.format(folder_name)
    save_file = 'coarse'
    version = 'v2'

    img_names = os.listdir(folder_pth)
    ...
    for i, (img_name, img_path) in enumerate(zip(img_names, img_paths)):
        ...
        t0 = time.time()
        detections = bm_detector.get_detection(img_path)
        t1 = time.time()
        ...
        t4 = time.time()
        print("{}/{} - Detection: {}sec Pose: {}sec Rendering: {}sec".format(i+1, len(img_names), t1-t0, t2-t1, t4-t3))
        ...
        export_png(figures['pred_rendered'], filename = os.path.join(save_dir2, '{}-rendered-{}.png'.format(save_file, version)))
        export_png(figures['pred_overlay'], filename = os.path.join(save_dir2, '{}-overlay-{}.png'.format(save_file, version)))
