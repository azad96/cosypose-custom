...
def main():
    ...
    refiner_run_id = 'bop-tless-kuartis-refiner--689761'
    # refiner_run_id = None
    refiner_epoch = 20
    ...
    folder_name = 'objects_720_540'
    folder_pth = '/mnt/trains/users/azad/BM/inputs/cell_4cam/{}'.format(folder_name)
    save_dir = '/mnt/trains/users/azad/BM/deneme/cell_4cam/{}'.format(folder_name)
    model_type = 'coarse-v4'
    ...
    for i, (img_name, img_path) in enumerate(zip(img_names, img_paths)):
        ...
        t3 = time.time()
        print("{}/{} - Detection: {:.3f} s Pose: {:.3f} s Rendering: {:.3f} s".format(i+1, len(img_names), t1-t0, t2-t1, t3-t2))
        ...
        export_png(res, filename = os.path.join(save_dir, 'result{}.png'.format(i+1)))

        # save_dir2 = os.path.join(save_dir, img_name.split('.')[0])
        # os.makedirs(save_dir2, exist_ok=True)

        # export_png(figures['input_im'], filename = os.path.join(save_dir2, 'input.png'))
        # export_png(figures['detections'], filename = os.path.join(save_dir2, 'detections.png'))
