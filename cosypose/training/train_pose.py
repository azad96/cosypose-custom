...
from cosypose.scripts.run_cosypose_eval import (
    load_kuatless_detection_results, get_pose_meters)
...
def log(config, model,
    ...
    def save_checkpoint_interval10(model, epoch):
        ckpt_name = 'checkpoint_{}'.format(epoch)
        ckpt_name += '.pth.tar'
        path = save_dir / ckpt_name
        torch.save({'state_dict': model.module.state_dict(),
                    'epoch': epoch}, path)
    ...
    if epoch%10 == 0:
        save_checkpoint_interval10(model, epoch)
...
def make_eval_bundle(args, model_training):
    ...
    if args.train_refiner:
        ...
        coarse_model = load_model(args.coarse_run_id_for_test, args.coarse_test_epoch)
    elif args.train_coarse:
        coarse_model = model_training
        refiner_model = load_model(args.refiner_run_id_for_test, args.refiner_test_epoch)
    else:
        raise ValueError
    ...
    for ds_name in args.test_ds_names:
        ...
        elif args.train_coarse and coarse_model is not None:
            pred_kwargs.update({
                det_k: dict(
